...
const serveStaticFile = (req, res) => {
  let filePath = path.join(PROJECT_ROOT, 'dist', req.url === '/' ? 'index.html' : req.url || '');
  
  // Handle client-side routing
  if (!fs.existsSync(filePath) || fs.statSync(filePath).isDirectory()) {
    filePath = path.join(PROJECT_ROOT, 'dist', 'index.html');
  }
  ...
  fs.readFile(filePath, (error, content) => {
    if (error) {
      if (error.code === 'ENOENT') {
        ...
        fs.readFile(path.join(PROJECT_ROOT, 'dist', 'index.html'), (err, content) => {
          if (err) {
            res.writeHead(500);
